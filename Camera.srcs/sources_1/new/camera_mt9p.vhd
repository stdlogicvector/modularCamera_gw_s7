...
--	FLASH_SCK_O		: out	STD_LOGIC := '0';
	...
	SENS_DATA_I		: in	STD_LOGIC_VECTOR(11 downto 0);
	
	SENS_OE_O		: out	STD_LOGIC := '0';
	SENS_TRIGGER_O	: out	STD_LOGIC := '0';
	SENS_nSTANDBY_O	: out	STD_LOGIC := '0';
	
	SENS_SCL_IO		: inout	STD_LOGIC := '0';
	SENS_SDA_IO 	: inout	STD_LOGIC := 'Z';
...
architecture toplevel of camera_mt9p is
...
signal i2c_data_r		: std_logic_vector(7 downto 0);
...
generic map (
	...
	DIVCLK_DIVIDE	=> 1,
	CLKFB_MULT		=> 16.0,
	CLK_OUT_DIVIDE	=> ( 0 => 8.0, 1 => 20.0, others => 0.0 )
)
port map (
	CLK_Ip	=> CLK50_I,
	
	CLK0_O	=> clk100,	-- 50MHz * 16 /  8 = 100MHz
	CLK1_O	=> clk40,	-- 50MHz * 16 / 20 =  40MHz
...
generic map (
	...
	DEFAULT_VALUE	=> (
						...
						2		=> int2vec(100000, 32)(15 downto  0),	-- Trigger Period in us (Lo)
						3		=> int2vec(100000, 32)(31 downto 16),	-- Trigger Period in us (Hi)
						4		=> int2vec(2, 16),						-- Exposure Time in us
						5		=> int2vec(2592, 16),					-- Test Image Width
						6		=> int2vec(1944, 16),					-- Test Image Height
						7		=> int2vec(0, 16),						-- Test Image H Blank in us
						8		=> x"1234",								-- Test Image Pattern
						9		=> int2vec(10, 8) & int2vec(1, 8),		-- Output V/H Blank
						10		=> int2vec(50000, 16),					-- Minimum Interval between frames
						11		=> int2vec(5000, 16),
						others	=> x"0000"
						)
)
...
port map (
	...
	nRST_O		=> SENS_nRESET_O,
	nSTANDBY_O	=> SENS_nSTANDBY_O,
